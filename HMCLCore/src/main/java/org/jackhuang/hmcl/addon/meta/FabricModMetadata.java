...
public final class FabricModMetadata {
    ...
    public static ModMetadata fromFile(Path modFile, ZipFileTree tree) throws IOException, JsonParseException {
        ZipArchiveEntry mcmod = tree.getEntryFlat("fabric.mod.json");
        if (mcmod == null)
        ...
        return new ModMetadata(metadata.id, ModLoaderType.FABRIC, metadata.name, new LocalAddonFile.Description(metadata.description),
