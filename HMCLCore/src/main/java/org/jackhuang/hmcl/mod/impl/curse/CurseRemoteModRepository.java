 ...
 */
package org.jackhuang.hmcl.mod.impl.curse;
...
public final class CurseRemoteModRepository implements RemoteModRepository {
    ...
    public CurseRemoteModRepository(Type type, int section) {
        this.type = type;
    ...
    public Stream<RemoteMod> search(String gameVersion, @Nullable RemoteModRepository.Category category, int pageOffset, int pageSize, String searchFilter, SortType sortType, SortOrder sortOrder) throws IOException {
        int categoryId = 0;
        if (category != null) categoryId = ((CurseMod.Category) category.getSelf()).getId();
        Response<List<CurseMod>> response = HttpRequest.GET(PREFIX + "/v1/mods/search",
                        pair("gameId", "432"),
                ...
                .getJson(new TypeToken<Response<List<CurseMod>>>() {
                }.getType());
        return response.getData().stream().map(CurseMod::toMod);
    }
    ...
    public RemoteMod getModById(String id) throws IOException {
        Response<CurseMod> response = HttpRequest.GET(PREFIX + "/v1/mods/" + id)
                .header("X-API-KEY", apiKey)
                .getJson(new TypeToken<Response<CurseMod>>() {
                }.getType());
    ...
    public RemoteMod.File getModFile(String modId, String fileId) throws IOException {
        Response<CurseMod.CurseVersionImpl> response = HttpRequest.GET(String.format("%s/v1/mods/%s/files/%s", PREFIX, modId, fileId))
                .header("X-API-KEY", apiKey)
                .getJson(new TypeToken<Response<CurseMod.CurseVersionImpl>>() {
                }.getType());
    ...
    public Stream<RemoteMod.Version> getRemoteVersionsById(String id) throws IOException {
        Response<List<CurseMod.CurseVersionImpl>> response = HttpRequest.GET(PREFIX + "/v1/mods/" + id + "/files",
                        pair("pageSize", "10000"))
                .header("X-API-KEY", apiKey)
                .getJson(new TypeToken<Response<List<CurseMod.CurseVersionImpl>>>() {
                }.getType());
        return response.getData().stream().map(CurseMod.CurseVersionImpl::toVersion);
    }

    public List<CurseMod.Category> getCategoriesImpl() throws IOException {
        Response<List<CurseMod.Category>> categories = HttpRequest.GET(PREFIX + "/v1/categories", pair("gameId", "432"))
                .header("X-API-KEY", apiKey)
                .getJson(new TypeToken<Response<List<CurseMod.Category>>>() {
                }.getType());
    ...
    public Stream<RemoteModRepository.Category> getCategories() throws IOException {
        return getCategoriesImpl().stream().map(CurseMod.Category::toCategory);
    }

    private List<CurseMod.Category> reorganizeCategories(List<CurseMod.Category> categories, int rootId) {
        List<CurseMod.Category> result = new ArrayList<>();

        Map<Integer, CurseMod.Category> categoryMap = new HashMap<>();
        for (CurseMod.Category category : categories) {
        ...
        for (CurseMod.Category category : categories) {
            if (category.getParentCategoryId() == rootId) {
                ...
                CurseMod.Category parentCategory = categoryMap.get(category.getParentCategoryId());
    ...
    public static final CurseRemoteModRepository MODS = new CurseRemoteModRepository(RemoteModRepository.Type.MOD, SECTION_MOD);
    public static final CurseRemoteModRepository MODPACKS = new CurseRemoteModRepository(RemoteModRepository.Type.MODPACK, SECTION_MODPACK);
    public static final CurseRemoteModRepository RESOURCE_PACKS = new CurseRemoteModRepository(RemoteModRepository.Type.RESOURCE_PACK, SECTION_RESOURCE_PACK);
    public static final CurseRemoteModRepository WORLDS = new CurseRemoteModRepository(RemoteModRepository.Type.WORLD, SECTION_WORLD);
    public static final CurseRemoteModRepository CUSTOMIZATIONS = new CurseRemoteModRepository(RemoteModRepository.Type.CUSTOMIZATION, SECTION_CUSTOMIZATION);
    ...
    private static class FingerprintMatch {
        ...
        private final CurseMod.CurseVersionImpl file;
        private final List<CurseMod.CurseVersionImpl> curseVersionImpls;

        public FingerprintMatch(int id, CurseMod.CurseVersionImpl file, List<CurseMod.CurseVersionImpl> curseVersionImpls) {
            this.id = id;
            this.file = file;
            this.curseVersionImpls = curseVersionImpls;
        }
        ...
        public CurseMod.CurseVersionImpl getFile() {
            return file;
        }

        public List<CurseMod.CurseVersionImpl> getLatestFiles() {
            return curseVersionImpls;
        }
    }
}
