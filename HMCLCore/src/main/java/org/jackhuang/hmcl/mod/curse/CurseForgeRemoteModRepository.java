...
public final class CurseForgeRemoteModRepository implements RemoteModRepository {
    ...
    public SearchResult search(String gameVersion, @Nullable RemoteModRepository.Category category, int pageOffset, int pageSize, String searchFilter, SortType sortType, SortOrder sortOrder) throws IOException {
        ...
        HttpRequest request = HttpRequest.GET(PREFIX + "/v1/mods/search",
                pair("gameId", "432"),
        ...
        Response<List<CurseAddon>> response = request.getJson(Response.typeOf(listTypeOf(CurseAddon.class)));
    ...
    public Optional<RemoteMod.Version> getRemoteVersionByLocalFile(LocalModFile localModFile, Path file) throws IOException {
        ...
        HttpRequest request = HttpRequest.POST(PREFIX + "/v1/fingerprints/432")
        ...
        Response<FingerprintMatchesResult> response = request.getJson(Response.typeOf(FingerprintMatchesResult.class));
    ...
    public RemoteMod getModById(String id) throws IOException {
        HttpRequest request = HttpRequest.GET(PREFIX + "/v1/mods/" + id);

        if (PREFIX.equals(DEFAULT_PREFIX)) {
            request.header("X-API-KEY", apiKey);
        }

        Response<CurseAddon> response = request.getJson(Response.typeOf(CurseAddon.class));
        return response.data.toMod();
    }

    @Override
    public RemoteMod.File getModFile(String modId, String fileId) throws IOException {
        HttpRequest request = HttpRequest.GET(String.format("%s/v1/mods/%s/files/%s", PREFIX, modId, fileId));
    ...
    public Stream<RemoteMod.Version> getRemoteVersionsById(String id) throws IOException {
        HttpRequest request = HttpRequest.GET(PREFIX + "/v1/mods/" + id + "/files",
                pair("pageSize", "10000"));
        ...
        Response<List<CurseAddon.LatestFile>> response = request.getJson(Response.typeOf(listTypeOf(CurseAddon.LatestFile.class)));
    ...
    public List<CurseAddon.Category> getCategoriesImpl() throws IOException {
        HttpRequest request = HttpRequest.GET(PREFIX + "/v1/categories", pair("gameId", "432"));

        if (PREFIX.equals(DEFAULT_PREFIX)) {
            request.header("X-API-KEY", apiKey);
        }

        Response<List<CurseAddon.Category>> categories = request.getJson(Response.typeOf(listTypeOf(CurseAddon.Category.class)));
