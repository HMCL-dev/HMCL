...
public final class AsyncTaskExecutor extends TaskExecutor {
        ...
        future = executeTasks(null, Collections.singleton(firstTask))
                .thenApplyAsync(exception -> {
                    ...
                    if (!success) {
                        ...
                        Logging.LOG.warning("An exception occurred in task execution", exception);
    ...
    public boolean test() {
        ...
        try {
            ...
            Logging.LOG.info("Task " + firstTask + " has been cancelled.", e);
        }
        return false;
    }
        ...
        return CompletableFuture.completedFuture(null)
                .thenComposeAsync(unused -> {
                    ...
                    if (task.getSignificance().shouldLog())
                        Logging.LOG.trace("Executing task: " + task.getName());
                ...
                .thenApplyAsync(result -> {
                    ...
                    if (task.getSignificance().shouldLog()) {
                        Logging.LOG.trace("Task finished: " + task.getName());
                    }
                ...
                .exceptionally(throwable -> {
                    ...
                    if (resolved instanceof Exception) {
                        ...
                        if (e instanceof InterruptedException || e instanceof CancellationException) {
                            ...
                            if (task.getSignificance().shouldLog()) {
                                Logging.LOG.trace("Task aborted: " + task.getName());
                            }
                            ...
                            exception = e;
                            if (task.getSignificance().shouldLog()) {
                                Logging.LOG.trace("Task failed: " + task.getName(), e);
                            }
                            task.onDone().fireEvent(new TaskEvent(this, task, true));
                            taskListeners.forEach(it -> it.onFailed(task, e));
                        }
    ...
    private <T> CompletableFuture<T> executeNormalTask(Task<?> parentTask, Task<T> task) {
        return CompletableFuture.completedFuture(null)
                ...
                .thenApplyAsync(dependenciesException -> {
                    ...
                    if (!isDependenciesSucceeded) {
                        Logging.LOG.error("Subtasks failed for " + task.getName());
