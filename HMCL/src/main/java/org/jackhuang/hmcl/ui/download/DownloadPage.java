...
public class DownloadPage extends DecoratorAnimatedPage implements DecoratorPage {
    ...
    public DownloadPage(String uploadVersion) {
        ...
        modpackTab.setNodeSupplier(loadVersionFor(() -> {
            DownloadListPage page = HMCLLocalizedDownloadListPage.ofModPack((profile, __, file, addon) -> {
                Versions.downloadModpackImpl(profile, uploadVersion, file, addon);
            }, false);
        ...
        modTab.setNodeSupplier(loadVersionFor(() -> HMCLLocalizedDownloadListPage.ofMod((profile, version, file, addon) -> download(profile, version, file, "mods"), true)));
        resourcePackTab.setNodeSupplier(loadVersionFor(() -> HMCLLocalizedDownloadListPage.ofResourcePack((profile, version, file, addon) -> download(profile, version, file, "resourcepacks"), true)));
        shaderTab.setNodeSupplier(loadVersionFor(() -> new DownloadListPage(ModrinthRemoteModRepository.SHADER_PACKS, (profile, version, file, addon) -> download(profile, version, file, "shaderpacks"), true)));
