...
public final class TaskListPane extends StackPane {
    ...
    private static class StageNode extends BorderPane {
        ...
        public StageNode(String stage) {
            ...
            setCenter(title);
            setLeft(FXUtils.limitingSize(SVG.DOTS_HORIZONTAL.createIcon(Theme.blackFillBinding(), 14, 14), 14, 14));
        }

        public void begin() {
            if (started) return;
            started = true;
            setLeft(FXUtils.limitingSize(SVG.ARROW_RIGHT.createIcon(Theme.blackFillBinding(), 14, 14), 14, 14));
        }

        public void fail() {
            setLeft(FXUtils.limitingSize(SVG.CLOSE.createIcon(Theme.blackFillBinding(), 14, 14), 14, 14));
        }

        public void succeed() {
            setLeft(FXUtils.limitingSize(SVG.CHECK.createIcon(Theme.blackFillBinding(), 14, 14), 14, 14));
        }
