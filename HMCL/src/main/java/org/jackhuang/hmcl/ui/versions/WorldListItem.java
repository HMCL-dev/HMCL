...
public class WorldListItem extends Control {
    ...
    public void manageDatapacks() {
        if (world.getGameVersion() == null || // old game will not write game version to level.dat
                (DefaultVersionNumber.isIntVersionNumber(world.getGameVersion()) // we don't parse snapshot version
                        && DefaultVersionNumber.asVersion(world.getGameVersion()).compareTo(DefaultVersionNumber.asVersion("1.13")) < 0)) {
