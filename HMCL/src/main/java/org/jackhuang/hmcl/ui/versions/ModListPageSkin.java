...

        {
            ...
            toolbarSelecting.getChildren().setAll(
                    ...
                    createToolbarButton2(i18n("mods.check_updates"), SVG.UPDATE, () -> {
                        // `listView.getSelectionModel().getSelectedItems()` should be executed in JFX thread
                        var mods = listView.getSelectionModel().getSelectedItems().stream().map(ModInfoObject::getModInfo).toList();
                        skinnable.checkUpdates(() -> mods);
                    }),
