...
public final class JavaInstallTask<F extends Closeable, E extends ArchiveEntry> extends Task<JavaManifest> {
    ...
    private final ArchiveFileTreeSupplier<F, E> fileSupplier;
    ...
    public JavaInstallTask(Path targetDir, Map<String, Object> update, ArchiveFileTreeSupplier<F, E> fileSupplier) {
    ...
    public void execute() throws Exception {
        ...
        try (ArchiveFileTree<F, E> tree = fileSupplier.open()) {
            ...
            ArchiveFileTree.Dir<E> javaRoot = tree.getRoot().getSubDirs().values().iterator().next();
            copyDirContent(tree, javaRoot, targetDir);
        }

        setResult(new JavaManifest(info, update, files));
    }

    private void copyDirContent(ArchiveFileTree<F, E> tree, ArchiveFileTree.Dir<E> dir, Path targetDir) throws IOException {
