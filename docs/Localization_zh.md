...
本文档介绍了 HMCLPP 对这些语言的支持状态，并给想要为 HMCLPP 的本地化工作做出贡献的贡献者提供了一份指南。
...
HMCLPP 使用符合 IETF BCP 47 规范的语言标签。
...
如果没有完全匹配当前语言环境的资源，HMCLPP 会根据当前环境的语言标签推导出一个搜索列表，根据该列表依次搜索资源。
...
HMCLPP 会要求所有 Pull Request 在更新文档和本地化资源时同步更新所有**主要**支持的语言对应的资源。
...
HMCLPP 欢迎任何人参与翻译和贡献。但是维护更多语言的翻译需要付出更多维护成本，所以我们对新增加的语言有一些要求。
请在贡献前确认以下要求:
...
如果你想为 HMCLPP 添加新的语言支持，请从翻译 [`I18N.properties`](../HMCL/src/main/resources/assets/lang/I18N.properties) 开始。
HMCLPP 的绝大多数文本都位于这个文件中，翻译此文件就能翻译整个界面。
...
在翻译了一部分后，你可以[自行构建 HMCLPP](./README_zh.md#编译)，编译出的 HMCLPP 中就会包含你的译文。
如果你的电脑默认环境不是该语言，你可以将环境变量 `HMCL_LANGUAGE` 指定为你刚刚从表格中找到的语言标签，
HMCLPP 会自动切换至这个语言。

到这里，你就可以把文件推送到 GitHub上，并向 HMCLPP 提交 PR 了。
HMCLPP 的维护者会替你完成其他步骤。
...
除了本地化文件，HMCLPP 还支持本地化 JSON 中的部分文本字段。JSON 中的本地化文本使用以下格式:
...
对于某个语言下的缺失的资源，HMCLPP 支持一套资源回退机制，会根据不同的语言标签推导出一个搜索列表，
根据该列表依次搜索资源。
...
1. 归一化语言代码
   ...
   如果当前语言标签中的语言代码子标签未在 IANA 语言子标签注册表中进行注册，HMCLPP 会先尝试将其映射为注册表中已注册的标签。
...
2. 映射宏语言至子语言
   ...
   且该宏语言通常指代某个个体语言，HMCLPP 会将其替换为该个体语言。

   例如 `zh` (中文) 通常实际指代 `cmn` (官话)，所以我们会将语言代码 `zh` 替换为 `cmn`。

3. 推导拼写脚本

   如果当前语言标签中未指定拼写脚本，HMCLPP 会依次根据以下规则尝试推导拼写脚本:
...
在对语言代码细化推导完成后，HMCLPP 会开始根据此语言标签推导出一个语言标签列表。

例如，对于语言标签 `en-US`，HMCLPP 会将其细化为 `en-Latn-US`，并据此推导出以下搜索列表:
...
对于语言标签 `zh-CN`，HMCLPP 会将其细化为 `cmn-Hans-CN`，并据此推导出以下搜索列表:
...
对于能够混合的资源 (例如 `.properties` 文件)，HMCLPP 会根据此列表的优先级混合资源；
对于难以混合的资源 (例如字体文件)，HMCLPP 会根据此列表加载找到的最高优先级的资源。

如果当前语言使用 ISO 639 标准的三字符标签，但同时也存在对应的两字符标签，那么 HMCLPP 会将其映射至两字符后再搜索资源。

例如，如果当前环境的语言标签为 `eng-US`，那么 HMCLPP 会将其映射至 `en-US` 后再根据上述规则搜索本地化资源。

### 对于中文的额外规则

HMCLPP 总是会将 `zh-CN` 加入所有中文环境的搜索列表中，将 `zh-TW` 加入所有繁体中文环境的搜索列表中。
...
为了简化文档的维护，HMCLPP 使用了一套宏机制自动维护文档的部分内容。在命令行中执行
