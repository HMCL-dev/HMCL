...
本文件介紹了 HMCL 對這些語言的支援狀態，並給想要為 HMCL 的本地化工作做出貢獻的貢獻者提供了一份指南。
...
在翻譯了一部分後，你可以[自行構建 HMCL](./README_zh.md#編譯)，編譯出的 HMCL 中就會包含你的譯文。
如果你的電腦預設環境不是該語言，你可以將環境變數 `HMCL_LANGUAGE` 指定為你剛剛從表格中找到的語言標籤，
HMCL 會自動切換至這個語言。

到這裡，你就可以把檔案推送到 GitHub上，並向 HMCL 提交 PR 了。
HMCL 的維護者會替你完成其他步驟。
