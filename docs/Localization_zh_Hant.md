...
本文件介紹了 HMCLPP 對這些語言的支援狀態，並給想要為 HMCLPP 的本地化工作做出貢獻的貢獻者提供了一份指南。
...
<summary>關於語言標籤</summary>
 ...
 HMCLPP 使用符合 IETF BCP 47 規範的語言標籤。
...
如果沒有完全匹配目前語言環境的資源， HMCLPP 會根據目前環境的語言標籤推導出一個搜尋列表，根據該列表依次搜尋資源。
...
而對使用者占比更低的資源使用更具體的 `zh-Hans`/`zh-Hant` 作為在地化機碼。
 ...
 HMCLPP 會要求所有 Pull Request 在更新文件和在地化資源時同步更新所有**主要**支援的語言對應的資源。
...
## 新增新的語言支援
 ...
 HMCLPP 歡迎任何人參與翻譯和貢獻。但是維護更多語言的翻譯需要付出更多維護成本，所以我們對新增加的語言有一些要求。
請在貢獻前確認以下要求:
...
如果你想為 HMCLPP 新增新的語言支援，請從翻譯 [`I18N.properties`](../HMCL/src/main/resources/assets/lang/I18N.properties) 開始。
 HMCLPP 的絕大多數文字都位於這個檔案中，翻譯此檔案就能翻譯整個介面。
...
在確定了語言標籤後，請在 [`I18N.properties` 檔案旁](../HMCL/src/main/resources/assets/lang)建立 `I18N_<在地化檔案後綴>.properties` (例如 `I18N_en.properties`) 文件。
隨後，你就可以開始在這個檔案中進行翻譯工作了。

`I18N.properties` 檔案會遵循[資源回退機制](#資源回退機制)查詢缺失的譯文。
也就是說，你可以逐條目進行翻譯，而你尚未翻譯的條目會自動回退到英語上。

在翻譯了一部分後，你可以[自行構建 HMCLPP](./README_zh.md#編譯)，編譯出的 HMCLPP 中就會包含你的譯文。
如果你的電腦預設環境不是該語言，你可以將環境變數 `HMCL_LANGUAGE` 指定為你剛剛從表格中找到的語言標籤，
 HMCLPP 會自動切換至這個語言。

到這裡，你就可以把檔案推送到 GitHub上，並向 HMCLPP 提交 PR 了。
 HMCLPP 的維護者會替你完成其他步驟。
...
除了在地化檔案， HMCLPP 還支援在地化 JSON 中的部分文字欄位。JSON 中的在地化文字使用以下格式:
...
對於某個語言下的缺失的資源， HMCLPP 支援一套資源回退機制，會根據不同的語言標籤推導出一個搜尋列表，
根據該列表依次搜尋資源。
...
1. 歸一化語言代碼
   ...
   如果目前語言標籤中的語言代碼子標籤未在 IANA 語言子標籤登錄檔中進行註冊， HMCLPP 會先嘗試將其映射為登錄檔中已註冊的標籤。

   例如，HMCL會將語言代碼 `eng` 取代為 `en`。

2. 映射巨集語言至子語言

   如果目前語言代碼是一個 [ISO 639 巨集語言](https://en.wikipedia.org/wiki/ISO_639_macrolanguage)，
   且該巨集語言通常指代某個個體語言， HMCLPP 會將其取代為該個體語言。

   例如 `zh` (中文) 通常實際指代 `cmn` (國語)，所以我們會將語言代碼 `zh` 取代為 `cmn`。

3. 推導拼寫指令碼

   如果目前語言標籤中未指定拼寫指令碼， HMCLPP 會依次根據以下規則嘗試推導拼寫指令碼:
...
在對語言代碼細化推導完成後， HMCLPP 會開始根據此語言標籤推導出一個語言標籤列表。

例如，對於語言標籤 `en-US`， HMCLPP 會將其細化為 `en-Latn-US`，並據此推導出以下搜尋列表:
...
對於語言標籤 `zh-CN`， HMCLPP 會將其細化為 `cmn-Hans-CN`，並據此推導出以下搜尋列表:
...
對於能夠混合的資源 (例如 `.properties` 檔案)， HMCLPP 會根據此列表的優先度混合資源；
對於難以混合的資源 (例如字體檔案)， HMCLPP 會根據此列表載入找到的最高優先度的資源。

如果目前語言使用 ISO 639 標準的三字元標籤，但同時也存在對應的兩字元標籤，那麼 HMCLPP 會將其映射至兩字元後再搜尋資源。

例如，如果目前環境的語言標籤為 `eng-US`，那麼 HMCLPP 會將其映射至 `en-US` 後再根據上述規則搜尋在地化資源。

### 對於中文的額外規則

 HMCLPP 總是會將 `zh-CN` 加入所有中文環境的搜尋列表中，將 `zh-TW` 加入所有繁體中文環境的搜尋列表中。
...
為了簡化文件的維護， HMCLPP 使用了一套巨集機制自動維護文件的部分內容。在命令列中執行
